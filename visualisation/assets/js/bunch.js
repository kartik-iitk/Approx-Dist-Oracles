...
function createRandomPoints() {
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    if (
      i > 0 &&
      squaredDist(newPoint, ORIGIN) <
        squaredDist(pointData[closestToOrigin], ORIGIN)
    ) {
      closestToOrigin = i;
    }
...
function generateBunches() {
  ...
  for (v in data.A[0]) {
    ...
    for (w in data.A[0]) {
      const d =
        v === w ? 0 : Math.sqrt(squaredDist(data.A[0][v], data.A[0][w]));
...
function drawCircle(center) {
  ...
  for (let [_, witness_id] of Object.entries(witnesses).reverse()) {
    ...
    const r =
      center.id != witness_id
        ? xscale(Math.sqrt(squaredDist(center, witness)))
        : 20;
    circle
      .append("circle")
...
function drawBunch(point) {
  ...
  for (let bunch_id of bunch) {
    ...
    drawLine(point, bunch_node, {
      class_id: `bunch_${point.id}`,
      color: "lightgrey",
    });
  }
}
...
function handleMouseOver(point) {
  ...
  drawCircle(data.v);
  labelPoint(
    data.A[0][Object.entries(data.p[point.id]).reverse()[0][1]],
    "p(v)"
  );
}
