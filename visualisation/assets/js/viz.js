...
const TOKEN =
  "pk.eyJ1IjoidHlsZXJ5ZXAiLCJhIjoiY2s3ZnQ5cGZmMDZmMTNvcGd1amFrOGV3ciJ9.32mOM4QHLL9QKl0TpUZvZw";
...
async function drawLines(projection) {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    const distance = latLongDistance(
      points[i][0],
      points[i][1],
      points[i + 1][0],
      points[i + 1][1]
...
function countPointsNearLine(p1, p2) {
  ...
  let filteredData = filteredData.filter(
    (d) =>
